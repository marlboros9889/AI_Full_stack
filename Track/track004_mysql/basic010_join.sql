...
alter table 테이블명 (add, drop, change/modify/rename )  -- 필드 자료형 옵션;

create table join_userinfo  select * from userinfo where 1=2;
alter table join_userinfo change no no int not null primary key;
alter table join_userinfo change no no int not null auto_increment;

desc join_userinfo;

insert into join_userinfo(no , name , age) value ( 1 , 'first'  ,  11 );
insert into join_userinfo(no , name , age) value ( 2 , 'second'  ,  22 );
insert into join_userinfo(no , name , age) value ( 3 , 'third'  ,  33 );
insert into join_userinfo(no , name , age) value ( 4 , 'first'  ,  44 );
...
--     필드명   자료형   옵션
-- );
--  필드명 : 영문소문자 ,  자료형 : int, varchar(100) , datetime  / 옵션 : not null, auto_increment (숫자자동증가) ,  primary key( 기본키)
use mbasic;
...
-- |  6 | sixth  | C    |
-- +----+--------+------+
insert into join_userban(no , name , ban) value ( 1 , 'first'  ,  'A' );
insert into join_userban(no , name , ban) value ( 2 , 'second'  ,  'B' );
insert into join_userban(no , name , ban) value ( 3 , 'third'  ,  'A' );
insert into join_userban(no , name , ban) value ( 4 , 'fourth'  ,  'C' );
insert into join_userban(no , name , ban) value ( 5 , 'fifth'  ,  'B' );
insert into join_userban(no , name , ban) value ( 6 , 'sixth'  ,  'C' );
