...
-- 1. 시스템의 현재 시각 조회 
select now(); -- 2026-05-26 14:37:15
select current_time(); -- 14:37:54
...
select date_format('2026-05-26 14:37:15' , "%Y-%m-%d");
...
select timestampdiff( hour, "2026-05-26","2026-05-25");   -- 24시간alter

-- now() weekday(0=월, 6=일), date_format(날짜,'%Y-%m-%d %H:%i-%s'), date_add
drop database date_userinfo;
 
show databases;
show tables like 'date_userinfo';
-- ■ 2. CRUD (SELECT Function-Date  연습문제)
-- >> 연습문제1)  date_userinfo
use mbasic;
use userinfo;
...
insert into date_userinfo (no,  name, age, `date`) values (4, 'ddd', 44,'2022-9-30 00:00:00');
DESC date_userinfo;
select * from date_userinfo;
-- mysql>
...
-- +------+----------------+
-- 4 rows in set (0.01 sec)
select  name, date_format(date, '%Y년 %m월 %d일') as`가입날짜`
from date_userinfo;
...
select date_add( now() , interval 5 day );  -- 수동으로 계산 05월 31일
select date_add( now() , interval (6- weekday( now() ) )day ); -- 내일이 와도 가장 오는 일요일

-- 지난 일요일
-- select date_add( now(0), interval( ));
...
INSERT INTO date_userinfo (no, name, age, `date`) VALUES (1, 'aaa', 11, '2022-12-30 00:00:00');
INSERT INTO date_userinfo (no, name, age, `date`) VALUES (2, 'bbb', 22, '2022-11-30 00:00:00');
INSERT INTO date_userinfo (no, name, age, `date`) VALUES (3, 'ccc',  3, '2022-10-30 00:00:00');
...
select count( if(sns='n', 1, null) )`수신거부수`
from if_userinfo;
