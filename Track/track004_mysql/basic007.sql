...
gno int 		not null auto_increment primary key,
...
from group_userinfo;
-- 10:42:26	select ban, count(*) from group_userinfo LIMIT 0, 2000	Error Code: 1140. In aggregated query without GROUP BY, expression #1 of SELECT list contains nonaggregated column 'mbasic.group_userinfo.ban'; this is incompatible with sql_mode=only_full_group_by	0.000 sec
select 		ban, count(*)		-- 반, 갯수
from 		group_userinfo
group by	ban;				-- 반별 (집계 필드 뺴고 전부)
...
-- +------+----------+----------+----------+
-- 3 rows in set (0.00 sec)
select	ban, sum(kor)`국어총합`, sum(eng)`영어총합`, sum(math)`수학총합`   -- ban(일반필드) sum(집계필드)
from group_userinfo
group by ban;
-- having
...
-- +------------+
-- 1 row in set (0.00 sec)
select count(*)`데이터갯수`   -- 집계함수는 null 값 빼고 집계 
from emp;
-- select count(*) `전체레코드`,count(empno)`모든데이터`,count(mgr)`null값뺴고`, 
...
-- +---------------+
-- 1 row in set (0.00 sec)
select count(*)`부서30 직원수`		-- 집계전 필터링
from emp
where  deptno = 30;

select count(*)`부서30 직원수`		-- 집계후 필터링
from emp
group by deptno
having deptno = 30;
-- --------------------------------------------------------
...
-- +---------------------+----------------+------------+
-- 1 row in set (0.00 sec)
select  count(distinct sal),  count(sal),  count(sal)
from emp;
...
-- +-----------------+
-- 1 row in set (0.00 sec)
select		max(sal)`부서10 최대급여`		-- 집계전 필터링
from		emp
where 		deptno = 10;

select		max(sal)`부서10 최대급여`		-- 집계후 필터링
from		emp
group by	deptno
having 		deptno = 10;
-- --------------------------------------------------------
...
-- +-----------------+
-- 1 row in set (0.00 sec) 
select		min(sal)`부서10 최소급여`		-- 집계 전 필터링
from		emp
where		deptno = 10;  

select		min(sal)`부서10 최소급여`		-- 집계 후 필터링
from		emp
group by	deptno
having 		deptno = 10;
...
select		deptno, job, avg(sal)`평균급여`
from		emp
-- where	sal >= 2000       직꼐합수의 조건은 having에서
group by	deptno, job
having avg(sal) > 2000
order by 	deptno, job;
-- --------------------------------------------------------
