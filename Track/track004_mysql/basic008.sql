...
-- 2-1. userinfo 테이블이 있으면 → 그대로 수정
alter table userinfo modify age int null;

-- 2-2. userinfo_select 이름으로 만들고 싶으면 → 이름 변경 후 수정
rename table userinfo to userinfo_select;
alter table userinfo_select modify age int null;
...
insert into userinfo_select values (1,'first',11),(2,'second',22),(3,'third',33),(4,'fourth',44);
...
select *
from userinfo_select
where age is not null 		-- 나이가 null 이 아니고 
and no in(2,3) 				-- no가 2또는 3중에서 
and name not like '_e%';	-- 이름의 두번째 글자가 e가 아닌 데이터 조회


-- Q8.  emp 테이블에서 GROUP BY절만  사용하여
-- -- 각부서의 직책별 사원수, 가장높은 급여, 급여합, 평균급여를  다음과 같이  사원데이터를  조회하시오.
select deptno, job, count(*)`사원수`, max(sal)`최그급여`, sum(sal)`급여합`, avg(sal)`평균급여`
from emp
group by deptno , job
order by deptno , job;
-- >> 결과
...
select 12.2345
, round(12.2545 ,1 )		-- 소수점 1번쨰 자리까지 반올림  12.3
, round(15.12345,-1)		-- 정수  1번째 자리에서 반올림  20
from dual;
...
select 123.45678,
		round(123.45678,  0)`ROUND1  0`,
		round(123.45678, -1)`ROUND2 -1`,
        round(123.45678,  1)`ROUND3  1`,
        round(123.45678,  2)`ROUND4  2`
from dual;
...
from dual;
-- upper( 소문자를 대문자로 up), lower(대문자를 소문자로 low)
-- +---------+------+------+
-- | DEFAULT | s1   | s2   |
-- +---------+------+------+
-- | ABC     | ABC  | abc  |
...
select replace('hello sally' , 'sally','a')  -- 문자열에서, 문자 찾아서, 바꾸기
from dual;
-- +--------------------------------------+
...
-- mysql> -- 10. 빈칸채우기 (어떠한 문자열, 갯수)
select	lpad('ABC' , 5, '#'), -- 왼쪽열에서부터  전체5칸수 안에   # 채우기
		rpad('ABC' , 5, '#')  -- 오른쪽열에서부터 전체5칸수 안에   # 채우기
from dual;
-- +----------------------+----------------------+
