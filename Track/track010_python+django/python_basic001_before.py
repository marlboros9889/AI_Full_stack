...
# ==============================================================================
# 0) python.org 
#   vs code : Extensions - python Extensions Pack/ Python Indent
...
print(f"index의 -1번째 과일 : {fruits[-1]}") # -1 맨뒤의 요소 추출
...
# json 형태로 테이거 가공
user_info = {"name" : "홍길동", "role":"도적", "leval":5}
user_info["leval"]+= 1   #기존의 Key의 value수정 / 없으면 새로 Key-value 추가

print(f"딕셔너리{user_info}")
print(f"역할 : {user_info['role']}, 레벨:{user_info['leval']}")
...
print("\n\n--- [2] NumPy 기초 ---")     # a ze a 
import numpy as np
# np.array([])                  : 기존 파이썬 리스트릴 NumPy 배열로 변환
# np.zeros(shape)               : 지정크기만큼 0으로 채워진 배열 
# np.arrange(start, end)        : start부터 end전까지 연속된 배열 
...
print(f"1차원 배열: {arr1}")
print(f"Array Shape : {arr1.shape}")    # (5,) 1차원 배열 데이터가 5개 
...
# [행, 열] 범위 지정
# [start : end] start부터 end 까지 영역
# 범위 `:`    if 조건 :   ,  for i in range : 
# data_2d[ : , 0 ]   모든행(:)의 0번째 열(방문자수 150, 300, 500)
...
print( data_2d[0 : 2])    #결과:  [[  150 10000] , [  300 50000]]
...
print(f"매출액 30000만 이상 필터링 : {sales_30000}")    # False, True, True 
...
print("방문자수 평균 :",   np.mean(visitors))    #결과: 316.6666666666667
print("총 매출액 합계 :",  np.sum(sales))        #결과: 120000
print("방문자수 최대 :",   np.max(visitors))     #결과: 500
print("매출액 표준편차 :", np.std(sales))        #결과: 21602.468994692867
...
col_sums = np.sum(data_2d , axis=0) #결과: [   950 120000 ]
...
# 주문 건수(0번 열)가 30건 이상인 매장의 데이터  
over_30_stores = store_data[store_data[:,0] >= 30] 
# sum  : 총 결제 금액(1번 열)  합계(`total_pay`)         
total_pay = np.sum(store_data[:, 1])  
# mean : 주문 건수(0번 열)의 평균(`avg_orders`)
avg_orders = np.mean(store_data[:, 0])   
               
# --- [출력 확인 공간] ---
print("\n--- [연습문제 02 결과 확인] ---")
print(f"Q1 결과 (주문 30건 이상 매장):\n", over_30_stores)
...
# ==============================================================================
# Pandas - 엑셀(Excel)이나 관계형 데이터베이스(Rdb)의 테이블형태
# 핵심객체
...
## head : 상단 미리보기 (기본값 5) 
## info : 전체 행 갯수, 컬럼 데이터 타입, Null(결측치) 존재여부 확인
...
# tolist() : 파이썬 List `[]`  포맷변환  Chart.js, 그래프그릴떄
