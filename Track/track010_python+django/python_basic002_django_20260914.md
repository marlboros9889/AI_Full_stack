...
```bash
#1. 프로젝트별 독립된 파이썬 라이러리 실행환경 (virtual environment)
# 작업 폴더 이동 후 가상환경 생성 
python  -m  venv  venv
...
# 답: 1) (  비즈니스 로직처리    ) / 2) (   db 와 tempalte 연결    )
...
urlpatterns = [
    ...
    path( '' , views.dashboard_view, name='dashboerd'),
]
