...
``` 
```
public ip : 13.209.17.105

ssh 클라이언트
> ssh -i "thejoa703.pem" ubuntu@ec2-13-209-17-105.ap-northeast-2.compute.amazonaws.com

# 1. icacls "thejoa703.pem" /inheritance:r /grant:r "$($env:USERNAME):(R)"
...
5. ECR 리포지토리
- 애플리케이션을 docker이미지로 빌드해서 ecr에 올려두면 어디서든지 가져다가 사용 가능 할수있게
※  ECR 검색
1) 리포지토리 이름
2) 이미지 태그 설정 - Mutable(연습용)
3) 암호화설정 - 기본키 그대로
```
URL : 087470309204.dkr.ecr.ap-northeast-2.amazonaws.com/thejoa703
```
...
#      사용자 계정    시스템그룹 docker  
#     현재로그인한 사용자에게 docker 그룹 권한을 줘서 sudo없이 docker 명령어 사용가능
```
...
```bash
    ...
    mkdir -p  /home/ubuntu/app/back/build/libs/upload

    #2. 홈 프로젝트 기본디렉토리 권한설정 (소유자는 모든 권한 rwx 421, 그룹/타인은 읽기/실행권한 rwx 4-1)
    sudo chmod  755  /home/ubuntu
    sudo chmod  755  /home/ubuntu/app
    sudo chmod  755  /home/ubuntu/app/back/build/libs/upload
    sudo chmod  644  /home/ubuntu/app/back/build/libs/upload/*   #이미 생성
```
...
10) docker - oracle 컨데이서 실행 ( pull, run, ps, start )
```
...
7. IM 사용자 / 역할 생성
1) IAM 콘솔 → 사용자추가
2) 권한정책: `AmazonEC2FullAccess`, `AmazonECS_FullAcces`, `AmazonEC2ContainerRegistryFullAccess`
3) Access Key / Secret Key 발급
4) Github Secrets 에 저장
- `AmazonEC2FullAccess`   →  EC2 인스턴스 관리 
- `AmazonECS_FullAccess`     →  ECS/Faragate 서비스관리
    > Github Actions, Jenkins 외부도구에서 빌드한 docker이미지를 AWS ECR에 올리고
    > 자동배포해주는 권한셋팅
...
>> https://github.com/marlboros9889/track009_aws.git

2. Action secrets and variables 시크릿 키 설정 
````
Name: EC2_HOST
Secret: <EC2_PUBLIC_IP>
# 현재 EC2 퍼블릭 IP를 입력

Name: EC2_USER
Secret: <EC2_LOGIN_USER>
...
# IAM에서 발급받은 Access Key를 입력

Name: AWS_SECRET_ACCESS_KEY
Secret: <AWS_SECRET_ACCESS_KEY>
# IAM에서 발급받은 Secret Key를 입력
...
# AWS 계정 ID 12자리 숫자를 입력

Name: ECR_REPO
Secret: <ECR_REPOSITORY_NAME>
# ECR 저장소 이름을 입력
````
part2
````
...
Secret: https://<DEPLOYED_DOMAIN_OR_IP>
 
```
※ ssh 클라이언트
> ssh -i "thejoa703.pem" ubuntu@ec2-3-37-86-84.ap



## Part002.Aws

■Step3. Github 워크플로우 작성 및 프로젝트 올리기
1. 구조확인
```
...
```
git clone https://github.com/marlboros9889/track009_aws.git
