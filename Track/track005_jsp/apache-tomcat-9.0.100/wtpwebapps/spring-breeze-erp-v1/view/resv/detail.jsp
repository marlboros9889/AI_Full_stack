...
<main class="sb-content">
  <%-- 자원 상세 화면: 일반 사용자는 조회/예약만, 관리자는 수정/삭제까지 가능하다. --%>
...
<c:if test="${isAdmin}">
  <%-- 상세 화면에서도 같은 삭제 경로를 사용하므로, returnTo 값으로 복귀 위치를 구분한다. --%>
  ...
  <script>
    ...
    // 상세 화면의 삭제 버튼 클릭 시 삭제 대상 자원 ID를 모달에 바인딩한다.
    ...
    // 비밀번호 확인 후 숨겨진 deleteForm 을 제출해 실제 삭제를 요청한다.
