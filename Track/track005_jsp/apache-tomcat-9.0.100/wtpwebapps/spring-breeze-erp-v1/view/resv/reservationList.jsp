...
<main class="sb-content">
  <%-- 예약 목록 화면: /reservation/list 에서 열리며 상태 필터와 페이지 이동을 처리한다. --%>
  <div class="sb-page-head">
    <div class="sb-page-head__txt">
      ...
      <h1>예약 내역</h1>
      <p>내가 요청한 자원 예약과 처리 상태를 확인할 수 있습니다.</p>
      <sec:authorize access="hasRole('ROLE_ADMIN')">
        <span class="sb-badge sb-badge--blue mt-2 d-inline-block">관리자 화면</span>
      </sec:authorize>
      <sec:authorize access="!hasRole('ROLE_ADMIN')">
        <span class="sb-badge sb-badge--green mt-2 d-inline-block">사용자 화면</span>
      </sec:authorize>
    </div>
  ...
  <c:if test="${error == 'notAllowed'}">
    <div class="alert alert-warning">대기 상태의 본인 예약만 수정하거나 삭제할 수 있습니다.</div>
  </c:if>
  ...
  <%-- 예약 목록 페이징: 상태 필터를 유지한 채 이전/다음 및 번호 이동을 제공한다. --%>
  <nav class="mt-3">
