...
<main class="sb-content">
  <%-- 자원 목록 진입 화면: 검색, 페이지 이동, 관리자 전용 관리 버튼을 함께 보여준다. --%>
  <div class="sb-page-head">
    <div class="sb-page-head__txt">
      ...
      <h1>자원 관리</h1>
      <p>회의실과 장비 같은 예약 가능 자원을 조회하고 관리합니다.</p>
  ...
  <c:if test="${error == 'hasReservations'}">
    <div class="alert alert-danger">예약 이력이 있는 자원은 예약 내역을 먼저 정리한 뒤 삭제할 수 있습니다.</div>
  </c:if>
  ...
  <div class="row g-3 mb-3">
    <div class="col-6 col-md-4">
      <div class="sb-card p-3 d-flex align-items-center gap-3">
        ...
        <div>
          <div class="text-faint" style="font-size:12px">전체 등록 자원</div>
    ...
    <div class="col-6 col-md-4">
      <div class="sb-card p-3 d-flex align-items-center gap-3">
        <span class="sb-stat__ico tone-violet"><i class="bi bi-person-badge"></i></span>
        <div>
          <div class="text-faint" style="font-size:12px">현재 화면</div>
          <div style="font-weight:750;font-size:17px">${roleLabel}</div>
        </div>
      </div>
    </div>
      ...
      <div class="sb-card p-3 d-flex align-items-center gap-3">
        ...
        <div>
          ...
          <div style="font-weight:750;font-size:17px">
            <c:choose>
              <c:when test="${isAdmin}">
                <a href="${pageContext.request.contextPath}/admin/approval/list">승인 관리로 이동</a>
              </c:when>
              <c:otherwise>관리자 전용</c:otherwise>
            </c:choose>
          </div>
        </div>
      </div>
  ...
  <div class="sb-card">
    <div class="sb-card__head">
      <h2>자원 목록</h2>
      <span class="sub">검색 조건에 맞는 자원을 확인할 수 있습니다.</span>
    </div>
  ...
  <%-- 목록 페이징: 현재 페이지를 기준으로 이전/다음 이동과 번호 이동을 제공한다. --%>
  <nav class="mt-3">
...
<c:if test="${isAdmin}">
  <%-- 관리자만 삭제 모달과 실제 삭제 POST 폼을 사용할 수 있다. --%>
  ...
  <script>
    ...
    // 목록의 삭제 버튼이 눌리면 자원 ID와 돌아갈 화면 정보를 모달에 세팅한다.
    ...
    // 모달에서 입력한 비밀번호를 숨은 폼에 복사한 뒤 /resv/delete 로 제출한다.
