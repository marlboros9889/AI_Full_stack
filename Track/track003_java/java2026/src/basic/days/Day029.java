...
public class Day029 {
	public static void main(String[] args) {
		...
		//Map	:	기차(key:value-쌍-엔트리)	put, get(key), size, remover, contains
		// Q2. ArrayList 출력
		...
		// Q3. List는 순서가 있고 중복을 허용하므로 Bowser가 2개 출력됨
		...
		while(iter.hasNext()) { //2. 처리대상의 유무
			Player player = iter.next(); //3. 한개씩 꺼내기
			System.out.printf("%d   %s  %d\n", i+1, player.getName(), player.getScore());
		}
		// Set은 중복을 허용하지 않으므로 Bowser는 1개만 출력됨
		...
		// for-each + entrySet 이용해서 출력
		for(String key : mapPlayers.keySet()) {
			Player player = mapPlayers.get(key);
			System.out.printf("%s  %s  %d\n", key, player.getName(), player.getScore());
		}
		System.out.println();
		...
		// 6-1. 익명 클래스로 점수 오름차순 정렬
		System.out.println("\n[1] 익명 클래스 - 점수 오름차순");
		players.sort(new Comparator<Player>() {
			@Override
			public int compare(Player o1, Player o2) {
				return Integer.compare(o1.getScore(), o2.getScore());//오름차순
			  //return Integer.compare(o1.getScore(), o2.getScore());//내림차순	
			}
		});
		...
		//for(int i=0; i<players.size(); i++) {
		// Player p = player.get(i);ystem.out.printf("%s  %d\n", player.getName(), player.getScore()); }
