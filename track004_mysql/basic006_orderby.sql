...
delete from userinfo where no >= 5; -- 5번이상 데이터 삭제
...
insert into select_userinfo (no , name ) values (7, 'seven' );  -- insert into -- 추가
 


-- mysql> select * rfrom select_userinfo;
-- +----+--------+------+
-- | no | name   | age  |
-- +----+--------+------+
-- |  1 | first  |   11 |
...
from select_userinfo
order by age desc
limit  0, 2; -- 어디서부터 , 몇개  ( 최신 부터, 2개) 66, 55

select no, age
from select_userinfo
order by age desc
limit  1, 2; -- 어디서부터, 몇개 ( 1번쨰 부터, 2개) 55, 44
