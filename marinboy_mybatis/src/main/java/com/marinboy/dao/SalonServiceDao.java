...
public interface SalonServiceDao {
    ...
    // 신규 메뉴와 이미지가 같은 ID를 사용하도록 다음 메뉴 ID를 조회합니다.
    Long findNextServiceId();
    // 관리자가 입력한 시술 메뉴 기본 정보를 저장합니다.
    int insertService(@Param("id") Long id, @Param("name") String name, @Param("category") String category, @Param("durationMinutes") int durationMinutes, @Param("price") int price, @Param("description") String description);
