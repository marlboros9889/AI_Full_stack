...
public class Sboard2SeriveImpl  implements Sboard2Service{
   ...
   //4. 입력기능 ( 이미지 올리기 )
   @Override public int insert(MultipartFile file, Sboard2Dto dto) throws IOException {
      // 이미지 업로드기능
      if(!file.isEmpty()) {
   ...
   //6. 수정기능 ( 이미지 올리기 )
   @Override public int update(MultipartFile file, Sboard2Dto dto) throws IOException {
      // 이미지 업로드기능
      if(!file.isEmpty()) {
         dto.setBfile( upload.fileUpload(file)  );
      }
